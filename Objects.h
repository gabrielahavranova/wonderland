...
class Mush : public MushroomBase {
 public:
  Mush(const float* vertices, const int vertices_cnt,
       const unsigned int* indices, const int indices_cnt,
       std::shared_ptr<Shader> shader, const std::string& tex_name,
       std::vector<glm::vec3>& colliders)
...
class Mushrooms : public MushroomBase {
 public:
  Mushrooms(const float* vertices, const int vertices_cnt,
            const unsigned int* indices, const int indices_cnt,
            std::shared_ptr<Shader> shader, const std::string& tex_name,
            std::vector<glm::vec3>& colliders)
      : MushroomBase(vertices, vertices_cnt, indices, indices_cnt, shader,
...
class God : public ObjectBase {
 public:
  ...
  void DrawObject() override {
    ...
    model =
        glm::rotate(model, glm::radians(25.0f), glm::vec3(0.0f, 1.0f, 0.2f));
    ...
    //	setMeshMaterial(glm::vec3(1.0f - color, color, 0.2f + color / 4.0f),
    //0.3f); 	for (const auto& mesh : meshes) { 		mesh.Draw();
    //	}
  }
};
...
class LightBlueBox : public ObjectBase {
 public:
  LightBlueBox(const float* vertices, const int vertices_cnt,
               const unsigned int* indices, const int indices_cnt,
               std::shared_ptr<Shader> shader)
      : ObjectBase(vertices, vertices_cnt, indices, indices_cnt, shader) {}
...
class Stars : public ObjectBase {
 public:
  ...
  void DrawObject() override {
    ...
    // this->position = glm::vec3(this->position.x + s*glm::cos(angle) * radius,
    // this->position.y + s*glm::sin(angle) * radius, this->position.z);

    // std::cout << "x: " << this->position.x + glm::cos(s * angle) * radius <<
    // ", y: " << this->position.y +  glm::sin(s * angle) * radius << ", z: " <<
